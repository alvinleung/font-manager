...
struct FontSync {
    ...
    static func syncWithPermission() async {
        ...
        let fontDirURL = fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Fonts", isDirectory: true)
        ...
        guard let syncSubDir = ensureDirExists(at: fontRootDir.url.appendingPathComponent("Sync")) else {
        ...
        guard let syncTargetDir = ensureDirExists(at: syncSubDir.appendingPathComponent(copyFromDir.url.lastPathComponent)) else {
            fontRootDir.endAccess()
            fontRootDir.endAccess()
    ...
    private static func syncDir(fromDir:URL, toDir:URL) async {
        ...
        for from in fromDirFontFiles {
            ...
            let fileWithEqualName = toDirFontFiles.first(where: { $0.lastPathComponent == from.lastPathComponent })
        ...
        for to in toDirFontFiles {
            let fileWithEqualName = fromDirFontFiles.first(where: { $0.lastPathComponent == to.lastPathComponent })
    ...
    private static func listSubDirectory(dir: URL) -> [URL] {
        ...
        guard let contents = try? fileManager.contentsOfDirectory(at: dir, includingPropertiesForKeys: [.isDirectoryKey], options: [.skipsHiddenFiles]) else {
